...
*
* @author Re-worked by Adam Courtemanche <adam.courtemanche@gmail.com>
* - Adds 'raw' params to a few functions
* - Adds addop() to class to add OR and - operators
*/
class TwitterSearch {
    /**
     ...
     * @var array
     */
    var $headers=array('X-Twitter-Client: PHPTwitterSearch','X-Twitter-Client-Version: 0.1','X-Twitter-Client-URL: ' . T_CLIENT_URL);
    
    /**
     * Recommend setting a user-agent so Twitter knows how to contact you inc case of abuse. Include your email
     * @var string
     */
	var $user_agent=T_USER_AGENT;
    ...
    */
    ...
    /**
	* Inject an OR or a negation operator (-)
    * @param string $op required
	*   The operator to inject
    * @return object
    */
    function addop($op) {
		if ($op == 'OR' || $op == '-') {
	...
	*   Should the results be returned raw (json|xml) or as a stdClass object
    * @param $reset_query boolean optional.
    * @return object
    */
    function results($raw = TRUE, $reset_query=TRUE) {
