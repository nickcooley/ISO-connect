...
isobar.Master = Ext.extend(Ext.Panel, {
    ...
    */
        ...
        if (localStorage.getItem('isoConnectUnlocked')) {
            ...
            this.authorPanel = new iso.channelLocked();
            if(!Ext.is.Phone){this.authorPanel.updateChannel(0)}
            this.channelList.addClass("chListLocked");
            this.authorPanel.on('beginUnlock', this.beginUnlock, this);
        }
...
iso.Util = {
  openUrl: function(url) {                
      ...
      Ext.Msg.confirm('External Link', 'This link will open in an external browser window. Would you like to continue?', 
         function(){
            console.log(url);
            //window.open("http://www.twitter.com/Weight_Loss2", "_news")
